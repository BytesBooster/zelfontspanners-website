              ...
              {result.success ? (
                <>
                  ...
                  {result.skipped !== undefined && result.skipped > 0 && (
                    <p style={{ color: '#b8b8b8', marginTop: '0.5rem' }}>
                      ⏭️ {result.skipped} foto's overgeslagen (al aanwezig)
                    </p>
                  )}
                  ...
                  {result.skippedDetails && result.skippedDetails.length > 0 && (
                    <div style={{ marginTop: '1rem' }}>
                      <p style={{ color: '#b8b8b8', marginBottom: '0.5rem', fontSize: '0.9rem' }}>
                        Voorbeelden van overgeslagen foto's:
                      </p>
                      <ul style={{ color: '#b8b8b8', paddingLeft: '1.5rem', fontSize: '0.85rem' }}>
                  ...
                  {result.debugInfo && result.debugInfo.length > 0 && (
                    <div style={{ marginTop: '1rem', padding: '1rem', background: 'rgba(33, 150, 243, 0.1)', borderRadius: '8px' }}>
                      <p style={{ color: '#2196f3', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: 'bold' }}>
                        🔍 Debug Informatie:
                      </p>
                      <ul style={{ color: '#b8b8b8', paddingLeft: '1.5rem', fontSize: '0.85rem' }}>
                        {result.debugInfo.map((item, index) => (
                          <li key={index} style={{ fontFamily: 'monospace' }}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  )}
